...
[![AppVeyor Build Status](https://ci.appveyor.com/api/projects/status/github/earthlab/smapr?branch=master&svg=true)](https://ci.appveyor.com/project/mbjoseph/smapr)
...
- (Recommended): Use `set_smap_credentials('yourusername', 'yourpasswd')`. This 
will save your credentials for later use by default, and optionally overwrite 
existing credentials if `overwrite = TRUE`. 
...
- Use `Sys.setenv()` in your `.Rprofile` to set those environmental variables 
...
* Please [report any issues or bugs](https://github.com/earthlab/smapr/issues),
